...
本仓库用于按文件化 workflow 写作一篇关于 agentic RL 在 API/tool-use 场景下 reward design 的系统综述。当前状态是 plan ready：仓库结构、scope、schema、检索计划、执行计划、空矩阵和审计占位已经准备好，但尚未开始正式联网检索、逐篇抽取或综述写作。
...
3. 第一轮正式检索代表工作和 benchmark，写入 `sources/sources.jsonl` 与 `matrices/work_taxonomy.csv`。
4. 第二轮补全 2024-2026 年最新 agentic RL / tool-use RL / process reward 工作。
5. 基于已有 sources 逐篇抽取 reward 信息，生成 `notes/works/{id}.md`。
...
可以开始执行 Prompt 2，也就是第一轮正式资料搜集。正式跑之前不要跳过以下检查：
...
- `sources/missing_sources.md` 是否列出了必须尝试寻找的工作。
